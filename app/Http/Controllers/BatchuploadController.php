...
class BatchuploadController extends Controller
{
    ...
    //UPDATED BUT ERROR ARRAY STRING CONVERSION
    ...
    //     // Use Model Binding for Validation
    //     try {
    //         $site = new Sites($rowData);

    //         $validator = Validator::make($site->toArray(), [
    //             "name" => ['required', 'string', 'max:255'],
    //             "ph_level" => ['required', 'numeric', 'between:0,14'],
    //             "turbidity" => ['required', 'numeric', 'min:0'],
    //             "total_dissolved_solids" => ['required', 'numeric', 'min:0'],
    ...
    //         $site->status = $site->calculateWaterStatus();

    //         // Save the Site instance
    //         $site->save();
    //     } catch (\Exception $e) {
    //         $errors["Row " . ($index + 1)] = ['An unexpected error occurred: ' . $e->getMessage()];
    //         continue;
    //     }
    // }
    ...
    //     return response()->json(['status' => 'success', 'message' => 'Data uploaded successfully!']);
    // }
}
